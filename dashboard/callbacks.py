...
from datetime import datetime  # Keep datetime as it may be used elsewhere
...
@callback(
    [Output("portfolio-value-graph", "figure"), Output("portfolio-stats", "children")],
    [
...
def update_portfolio_value(n_intervals: int, timeframe: str) -> tuple:
    ...
    minutes = {"1H": 60, "4H": 240, "1D": 1440, "1W": 10080}.get(timeframe, 60)
    ...
    figure = {
        "data": [
            go.Scatter(
                x=df.index, y=df["total_value"], name="Portfolio Value", fill="tozeroy"
            )
        ],
        "layout": {
        ...
        position_data.append(
            {
                ...
                "entry_time": pd.to_datetime(pos["entry_time"]).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
        ...
        event_elements.append(
            html.Div(
                [
                    ...
                    html.Span(
                        f"{event_type}: ", style={"color": color, "fontWeight": "bold"}
                    ),
                    html.Span(message),
                ],
