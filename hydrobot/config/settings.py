...
class PathSettings(BaseModel):
    ...
    data_dir: str = Field(
        "data", description="Directory for storing data files (CSV, etc.)."
    )
    model_dir: str = Field(
        "models/saved", description="Directory to save trained models."
    )
    config_dir: str = Field(
        "config", description="Directory where configuration files are stored."
    )
...
class ExchangeAPISettings(BaseModel):
    ...
    name: str = Field(
        ..., description="Name of the exchange (e.g., 'binanceus', 'coinbase')."
    )
    api_key: Optional[SecretStr] = Field(None, description="API Key for the exchange.")
    api_secret: Optional[SecretStr] = Field(
        None, description="API Secret for the exchange."
    )
    api_passphrase: Optional[SecretStr] = Field(
        None, description="API Passphrase (if required, e.g., Coinbase Pro)."
    )
    is_sandbox: bool = Field(
        False, description="Whether to use the exchange's sandbox/test environment."
    )
...
class TradingSettings(BaseModel):
    ...
    default_order_type: str = Field(
        "limit", description="Default order type ('limit', 'market')."
    )
    default_trade_amount_usd: float = Field(
        10.0, description="Default amount in USD for trades."
    )
    max_open_positions: int = Field(
        3, description="Maximum number of concurrent open positions."
    )
    slippage_tolerance: float = Field(
        0.001, description="Allowed slippage percentage (0.1%)."
    )  # Example
...
class RiskSettings(BaseModel):
    ...
    max_drawdown_pct: float = Field(
        0.1, description="Maximum allowed portfolio drawdown percentage (10%)."
    )
    max_risk_per_trade_pct: float = Field(
        0.01, description="Maximum percentage of portfolio to risk per trade (1%)."
    )
    stop_loss_pct: Optional[float] = Field(
        0.02, description="Default stop loss percentage (2%). Set to None to disable."
    )
    take_profit_pct: Optional[float] = Field(
        0.04, description="Default take profit percentage (4%). Set to None to disable."
    )
...
class ModelInferenceSettings(BaseModel):
    ...
    prediction_model_path: Optional[str] = Field(
        None, description="Path to the trained prediction model file."
    )
    regime_model_path: Optional[str] = Field(
        None, description="Path to the trained market regime model file."
    )
    use_gpu: bool = Field(
        False, description="Whether to use GPU for inference if available."
    )
...
class RedisSettings(BaseModel):
    ...
    password: Optional[SecretStr] = Field(
        None, description="Redis password (if required)."
    )
...
class AppSettings(BaseModel):
    ...
    strategy_manager: StrategyManagerSettings = Field(
        default_factory=StrategyManagerSettings
    )
    scalping_strategy: ScalpingStrategySettings = Field(
        default_factory=ScalpingStrategySettings
    )
    momentum_strategy: MomentumStrategySettings = Field(
        default_factory=MomentumStrategySettings
    )
    ...
    model_inference: Optional[ModelInferenceSettings] = Field(
        default_factory=ModelInferenceSettings
    )  # Optional for now
...
# --- Loading Function ---
def load_config(
    config_file: str = "config/config.yaml", secrets_file: Optional[str] = None
) -> AppSettings:
    """
    ...
    if secrets_file and os.path.exists(secrets_file):
        ...
        with open(secrets_file, "r") as f:
            ...
            if "exchange" in secrets_data and isinstance(
            ...
            if "redis" in secrets_data and isinstance(secrets_data["redis"], dict):
                ...
                log.warning(
                    f"Skipping merge for 'redis' secrets: Expected a dictionary, found {type(secrets_data['redis'])}."
                )
            # --- End FIX ---
    ...
    if api_key_env:
        log.info(
            "Overriding Exchange API key from environment variable EXCHANGE_API_KEY."
        )
...
try:
    ...
    if os.path.exists(_default_config_path):
        ...
        raise FileNotFoundError(
            f"Configuration file '{_default_config_path}' not found."
        )
except Exception:
...
def get_config() -> AppSettings:
    ...
    if CONFIG is None:
        log.critical(
            "Configuration accessed before it was successfully loaded or loading failed!"
        )
