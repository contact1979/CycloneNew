...
class OrderBook:
    ...
    def update(self, bids: List[List[float]], asks: List[List[float]]) -> None:
        ...
        self.bids = sorted(
            [[p, a] for p, a in self.bid_dict.items()], key=lambda x: x[0], reverse=True
        )[: self.depth]

        self.asks = sorted(
            [[p, a] for p, a in self.ask_dict.items()], key=lambda x: x[0]
        )[: self.depth]
...
class MarketDataStream:
    ...
    async def initialize(self, symbols: List[str]) -> None:
        ...
        for symbol in symbols:
            ...
            # Create event queue
            self._event_queues[symbol] = asyncio.Queue(maxsize=self.max_queue_size)
            ...
            try:
                ...
                for event in events:
                    ...
                    if event_type == "orderbook":
                        self.orderbooks[symbol].update(data["bids"], data["asks"])
                        MARKET_DATA_UPDATES.labels(
                            symbol=symbol, type="orderbook"
                        ).inc()

                    elif event_type == "trade":
                        self.trade_buffers[symbol].add_trade(data)
                        MARKET_DATA_UPDATES.labels(symbol=symbol, type="trade").inc()
                ...
                logger.error("event_processing_error", error=str(e), symbol=symbol)
                await asyncio.sleep(1)

    async def _maintain_websocket(
        self, symbol: str, url: str, subscribe_msg: Dict[str, Any]
    ) -> None:
                        ...
                        if msg.type == aiohttp.WSMsgType.TEXT:
                            ...
                            try:
                                ...
                                if not event_queue.full():
                                    ...
                                    logger.warning("event_queue_full", symbol=symbol)
                            except Exception as e:
                                logger.error(
                                    "event_queue_error", error=str(e), symbol=symbol
                                )

                        elif msg.type == aiohttp.WSMsgType.ERROR:
                            logger.error(
                                "websocket_error", error=str(msg.data), symbol=symbol
                            )
                            break

            except Exception as e:
                logger.error("websocket_connection_error", error=str(e), symbol=symbol)
    ...
    def get_market_state(self, symbol: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "trades": {
                ...
                "last_price": (
                    trade_buffer.trades[-1]["price"] if trade_buffer.trades else None
                ),
            },
        }
