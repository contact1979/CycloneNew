...
# Import HydroBot packages
from hydrobot import config
from hydrobot.database import (  # Import the db_utils module to access table objects and engine
    db_utils,
)
from hydrobot.database.db_utils import fetch_table_data
...
SENTIMENT_WINDOWS = [
    config.SENTIMENT_AGG_WINDOW_SHORT,
    config.SENTIMENT_AGG_WINDOW_LONG,
]  # e.g., ['1h', '24h']
PREDICTION_HORIZON = (
    config.PREDICTION_HORIZON_PERIODS
)  # e.g., 3 periods (15 mins for 5m interval)

# --- Helper Functions ---


def _calculate_rolling_sentiment(
    sentiment_df: pd.DataFrame, price_timestamps: pd.Series, window: str
) -> pd.DataFrame:
    ...
    if sentiment_df.empty:
        ...
        return pd.DataFrame(
            ...
            columns=[f"sent_avg_{window}", f"sent_count_{window}"],
        ).rename_axis("open_time")

    # Ensure sentiment_df index is datetime
    if not pd.api.types.is_datetime64_any_dtype(sentiment_df.index):
        log.warning(
            f"Sentiment DataFrame index is not datetime for window {window}. Attempting conversion."
        )
        try:
            # Ensure the index is timezone-aware UTC before proceeding
            if sentiment_df.index.tz is None:
                sentiment_df.index = pd.to_datetime(sentiment_df.index).tz_localize(
                    pytz.utc
                )
            else:
                sentiment_df.index = pd.to_datetime(
                    sentiment_df.index, utc=True
                ).tz_convert(pytz.utc)
    ...
    rolling_count = (
        sentiment_df["sentiment_score"].rolling(window, closed="left").count()
    )

    # Combine and rename
    rolling_sentiment = pd.DataFrame(
        {f"sent_avg_{window}": rolling_avg, f"sent_count_{window}": rolling_count},
    ...
    log.debug(
        f"Calculated rolling sentiment for window: {window}. Shape: {aligned_sentiment.shape}"
    )
...
def _calculate_target(price_df: pd.DataFrame, periods: int) -> pd.Series:
    ...
    target = (future_close > price_df["close"]).astype(
        float
    )  # Use float 1.0 / 0.0, NaN where future unknown
...
def generate_features_for_symbol(
    ...
    history_duration: pd.Timedelta = pd.Timedelta(days=30),
    ...
    """
    if not end_time_utc.tzinfo or end_time_utc.tzinfo.utcoffset(
        end_time_utc
    ...
    log.info(
        f"Generating features for {symbol} from {start_time_utc} to {end_time_utc}"
    )
    ...
    try:
        ...
        log.info(
            f"Fetched {len(sentiment_df)} sentiment data points (all symbols)."
        )  # Note: All symbols
    ...
    for window in ROLLING_WINDOWS:
        ...
        features[f"roll_avg_close_{window}"] = (
            price_df["close"].rolling(window, closed="left").mean()
        )
        features[f"roll_std_close_{window}"] = (
            price_df["close"].rolling(window, closed="left").std()
        )
        features[f"roll_avg_vol_{window}"] = (
            price_df["volume"].rolling(window, closed="left").mean()
        )
    ...
    for col in indicator_cols:
        if col in price_df.columns:
            ...
            if col == "sma_slow" and "sma_fast" in features.columns:
                features["sma_diff"] = features["sma_fast"] - features["sma_slow"]
            if col == "ema_slow" and "ema_fast" in features.columns:
                features["ema_diff"] = features["ema_fast"] - features["ema_slow"]
            if col == "macd_signal" and "macd_line" in features.columns:
                features["macd_diff"] = features["macd_line"] - features["macd_signal"]
    ...
    if (
        ...
        for window in SENTIMENT_WINDOWS:
            rolling_sent = _calculate_rolling_sentiment(
                sentiment_df.copy(), features.index, window
            )  # Pass copy
    ...
    features = pd.concat([features, target], axis=1)    # --- 4. Cleanup ---
    ...
    # Minimum needed for lags and rolling windows
    initial_len = len(features)
    # Drop rows where longest lag OR target is NaN
    features.dropna(
        subset=[f"lag_close_{PRICE_LAG_PERIODS}", target_col_name], inplace=True
    )
    final_len = len(features)
    log.info(
        f"Feature generation complete for {symbol}. Shape: {features.shape}. Dropped {initial_len - final_len} initial/final rows with NaNs."
    )
    ...
    if features.isnull().values.any():
        ...
        log.warning(
            f"Features still contain NaNs after initial drop: {nan_cols}. Imputing with 0."
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Generating features for {test_symbol} up to {end_time} with {history} history..."
    )

    # Ensure DB is initialized (if running standalone)
    # db_utils.init_db() # Uncomment if needed, ensure config is correct

    features_df = generate_features_for_symbol(
        test_symbol, end_time, history_duration=history
    )
    ...
    if features_df is not None and not features_df.empty:
        ...
        print(
            "\nFailed to generate features for {}. Check logs and database connection/data.".format(test_symbol)
        )
