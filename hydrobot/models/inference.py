...
class ModelInference:
    ...
    def _should_train(self) -> bool:
        ...
        time_since_training = (
            datetime.utcnow() - self._last_training_time
        ).total_seconds()
    ...
    async def train_models(self, historical_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if price_trained and regime_trained:
                ...
                logger.info("models_trained", timestamp=self._last_training_time)
    ...
    async def get_predictions(
        self, features: Dict[str, float], market_data: Dict[str, Any]
        ...
        try:
            ...
            regime, _ = await asyncio.get_event_loop().run_in_executor(
                None,
                self.regime_classifier.detect_regime,
                market_data["prices"],
                market_data["volumes"],
            )
    ...
    def get_prediction_confidence(
        self, price_prediction: float, regime: MarketRegime
    ) -> float:
    ...
    async def save_models(self, paths: Dict[str, str]) -> bool:
        ...
        try:
            ...
            price_saved = await asyncio.get_event_loop().run_in_executor(
                None, self.prediction_model.save_model, paths.get("prediction_model")
            )

            # Save regime classifier
            regime_saved = await asyncio.get_event_loop().run_in_executor(
                None, self.regime_classifier.save_model, paths.get("regime_model")
            )
