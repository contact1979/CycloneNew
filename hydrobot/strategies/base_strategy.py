...
class Signal:
    ...
    price: Optional[float] = (
        None  # Target price for LIMIT orders, execution price hint for MARKET
    )
    ...
    )  # Extra info (e.g., indicators, reasons)
    ...
    def __str__(self):
        ...
        details = (
            f"{self.action} {self.quantity:.6f} {self.symbol}"
            if self.symbol and self.quantity
            else self.action
        )
...
class Strategy(ABC):
    ...
    def __init__(self, strategy_config: Dict[str, Any], global_config: Any):
        ...
        )  # Get the name of the specific strategy class
    ...
    def update_parameters(self, new_params: Dict[str, Any]):
        ...
        for key, value in new_params.items():
            if hasattr(self.config, key):  # Or check if key exists if config is dict
                ...
                )  # Assumes config is an object like Pydantic model
                # If config is a dict: self.config[key] = value
            else:
                log.warning(
                    f"  Attempted to update unknown parameter '{key}' for strategy '{self.strategy_name}'"
                )
