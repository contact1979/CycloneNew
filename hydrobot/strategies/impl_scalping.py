...
from .strategy_settings import ScalpingStrategySettings, StrategySettings
...
class ScalpingStrategy(Strategy):
    ...
    def __init__(
        self, strategy_config: ScalpingStrategySettings, global_config: AppSettings
    ):
    ...
    def _calculate_order_book_imbalance(
        ...
        log.debug(
            f"[{self.symbol}] Imbalance: Bid={bid_volume:.4f}, Ask={ask_volume:.4f}, Ratio={imbalance:.4f}"
        )
        return imbalance
    ...
    def generate_signal(
        self, market_data: Dict[str, Any], model_prediction: Optional[Any] = None
    ) -> Signal:
        """Generates a BUY, SELL, or HOLD signal."""
        signal = Signal(
            action="HOLD", symbol=self.symbol, strategy_name=self.strategy_name
        )
        if not self.symbol:
        ...
        if (
            ...
            or math.isnan(best_bid_price)
            or math.isnan(best_ask_price)
        ):
            log.warning(
                f"[{self.symbol}] Invalid BBO: Ask={best_ask_price}, Bid={best_bid_price}. Holding."
            )
            return signal
        ...
        if model_prediction is not None:
            ...
            confidence = (
                model_prediction.get("confidence", 0.5)
                if isinstance(model_prediction, dict)
                else 0.5
            )
            log.debug(f"[{self.symbol}] Using model confidence: {confidence:.2f}")
        elif (
            self.config.confidence_threshold is None
            or self.config.confidence_threshold <= 0
        ):
            confidence = 1.0
            log.debug(f"[{self.symbol}] Confidence check bypassed.")
        else:
            log.warning(
                f"[{self.symbol}] Confidence threshold required but no model prediction. Holding."
            )
            return signal
        ...
        if spread_pct > self.config.min_spread_pct:
            log.debug(
                f"[{self.symbol}] Spread {spread_pct:.4f}% > Min {self.config.min_spread_pct:.4f}%. Holding."
            )
            return signal
        if (
            ...
            log.debug(
                f"[{self.symbol}] Confidence {confidence:.2f} < Threshold {self.config.confidence_threshold}. Holding."
            )
            return signal
        ...
        if imbalance >= self.config.min_imbalance:
            log.info(
                f"[{self.symbol}] BUY condition met: Imb={imbalance:.2f}, Spread={spread_pct:.4f}%"
            )
            signal.action = "BUY"
            signal.price = best_ask_price
            signal.confidence = confidence
            signal.stop_loss_price = (
                signal.price * (1 - self.global_config.risk.stop_loss_pct)
                if self.global_config.risk.stop_loss_pct
                else None
            )
            signal.take_profit_price = signal.price * (
                1 + self.config.min_profit_target_pct
            )

        # Check Imbalance (Sell)
        elif imbalance <= (
            1 / self.config.min_imbalance
            if self.config.min_imbalance != 0
            else float("inf")
        ):
            log.info(
                f"[{self.symbol}] SELL condition met: Imb={imbalance:.2f}, Spread={spread_pct:.4f}%"
            )
            signal.action = "SELL"
        ...
        if (
            ...
            trade_value_usd = min(
                self.global_config.trading.default_trade_amount_usd,
                self.config.max_position_size_usd,
            )
            signal.quantity = trade_value_usd / signal.price
            log.info(
                f"[{self.symbol}] Generated Signal: {signal} (Qty: {signal.quantity:.8f})"
            )
        elif signal.action == "HOLD":
            pass
        else:
            log.warning(
                f"[{self.symbol}] Signal action '{signal.action}' error or invalid price. Reverting to HOLD."
            )
            signal.action = "HOLD"
    ...
    def update_parameters(self, new_params: Dict[str, Any]):
        ...
        log.info(
            f"Updating parameters for strategy '{self.strategy_name}' for symbol {self.symbol}"
        )
        for key, value in new_params.items():
            if hasattr(self.config, key):
                ...
                try:
                    ...
                    log.warning(
                        f"  Could not update {key}: Error converting value '{value}' to type {field_type}. {e}"
                    )
            else:
