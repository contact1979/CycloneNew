...
class StrategyManager:
    ...
    def __init__(self, config: AppSettings):
        ...
        self.available_strategies: Dict[str, Type[Strategy]] = (
            self._discover_strategies()
        )
        self.active_strategy_instances: Dict[str, Strategy] = {}
        log.info("StrategyManager initialized.")
        log.info(
            f"Available strategy classes found: {list(self.available_strategies.keys())}"
        )
    ...
    def _discover_strategies(self) -> Dict[str, Type[Strategy]]:
        ...
        prefix = (
            self.__module__.split(".")[0] + ".strategies."
        )  # Assumes hydrobot.strategies...
            ...
            try:
                ...
                for name, obj in inspect.getmembers(module, inspect.isclass):
                    if issubclass(obj, Strategy) and obj is not Strategy:
                        log.debug(
                            f"Discovered Strategy class: {name} in module {modname}"
                        )
                        if name in strategies:
                            log.warning(
                                f"Duplicate strategy class name '{name}' found."
                            )
                        strategies[name] = obj
            except Exception as e:
                log.error(
                    f"Failed to import or inspect module {modname}: {e}", exc_info=True
                )
    ...
    def _get_strategy_class_for_regime(
        self, regime: str = "default"
        ...
        if not strategy_name:
            ...
            if not strategy_name:
                log.error(
                    f"No strategy mapping found for regime '{regime}' or 'default'."
                )
                return None
        ...
        if not strategy_class:
            log.error(
                f"Strategy class '{strategy_name}' not found in {list(self.available_strategies.keys())}"
            )
            return None
    ...
    def _get_strategy_config(
        ...
        config_key = strategy_class_name[0].lower() + strategy_class_name[1:].replace(
            "Strategy", "_strategy"
        )
        if hasattr(self.global_config, config_key):
            strategy_conf = getattr(self.global_config, config_key)
            log.debug(
                f"Found config for '{strategy_class_name}' under key '{config_key}'."
            )
            return strategy_conf
        else:
            log.warning(
                f"No specific config found for '{strategy_class_name}' (key: '{config_key}')."
            )
            return None

    def get_strategy_for_symbol(
        self, symbol: str, market_regime: str = "default"
    ) -> Optional[Strategy]:
        ...
        if not required_strategy_class:
            log.error(
                f"[{symbol}] Could not determine strategy class for regime '{market_regime}'."
            )
            return None
        ...
        if existing_instance and isinstance(existing_instance, required_strategy_class):
            ...
            log.info(
                f"[{symbol}] Regime changed. Replacing '{existing_instance.get_name()}' with '{required_strategy_name}'."
            )
        ...
        if strategy_config_data is None:
            log.warning(
                f"[{symbol}] No specific config for {required_strategy_name}. Using defaults."
            )
            strategy_config_data = {}  # Pass empty dict

        try:
            new_instance = required_strategy_class(
                strategy_config=strategy_config_data, global_config=self.global_config
            )
            ...
            log.exception(
                f"[{symbol}] Failed to instantiate strategy '{required_strategy_name}': {e}"
            )
            return None
    ...
    def update_strategy_parameters(self, symbol: str, new_params: Dict[str, Any]):
        ...
        if strategy_instance:
            log.info(
                f"[{symbol}] Requesting parameter update for '{strategy_instance.get_name()}'."
            )
