...
class TradingManager:
    ...
    ):  # Removed market_data_stream for now
    ...
    async def _handle_market_data_update(self, symbol: str, data: Dict[str, Any]):
        ...
        if (
            ...
            and bids[0]
            and asks[0]
            and isinstance(bids[0][0], (int, float))
            and not math.isnan(bids[0][0])
            and isinstance(asks[0][0], (int, float))
            and not math.isnan(asks[0][0])
        ):
    ...
    async def _run_trade_cycle(self, symbol: str, market_data: Dict[str, Any]):
        ...
        if not strategy:
            log.error(f"[{symbol}] No strategy found. Skipping.")
            return
        ...
        try:
            ...
            log.exception(f"[{symbol}] Signal generation error: {e}")
            return

        # --- 2. Validate Risk ---
        portfolio_value = self.position_manager.get_total_portfolio_value(
            self.latest_prices
        )
        log.debug(f"[{symbol}] Portfolio value for risk check: {portfolio_value:.2f}")
        try:
            validated_signal = self.risk_controller.validate_trade(
                signal, portfolio_value, self.latest_prices
            )
        except Exception as e:
            log.exception(f"[{symbol}] Risk validation error: {e}")
            return

        if not validated_signal:
            log.info(f"[{symbol}] Signal rejected by RiskController.")
            return
        ...
        if signal.action in ["BUY", "SELL"]:
            if signal.quantity is None or signal.quantity <= 0:
                log.error(
                    f"[{symbol}] Invalid signal qty ({signal.quantity}). Skipping."
                )
                return

            formatted_quantity = trading_utils.format_quantity(symbol, signal.quantity)
            formatted_price = (
                trading_utils.format_price(symbol, signal.price)
                if signal.price
                else None
            )

            if formatted_quantity is None or (
                signal.order_type == "LIMIT" and formatted_price is None
            ):
                log.error(
                    f"[{symbol}] Format error: Qty={formatted_quantity}, Price={formatted_price}. Skipping."
                )
                return
            if formatted_quantity <= 0 or (
                formatted_price is not None and formatted_price <= 0
            ):
                log.error(
                    f"[{symbol}] Invalid formatted values: Qty={formatted_quantity}, Price={formatted_price}. Skipping."
                )
                return

            is_size_valid = trading_utils.check_min_order_size(
                symbol, formatted_quantity, formatted_price
            )
            if not is_size_valid:
                log.warning(
                    f"[{symbol}] Order size {formatted_quantity} invalid. Skipping."
                )
                return
            ...
            log.debug(
                f"[{symbol}] Formatted params: Qty={signal.quantity}, Price={signal.price}"
            )
            ...
            try:
                ...
                order_result = await self.order_executor.execute_signal(
                    signal, self.latest_prices
                )
                if order_result:
                    log.info(
                        f"[{symbol}] Execution result: {order_result.get('status')}, ID: {order_result.get('id')}"
                    )
                else:
                    log.warning(f"[{symbol}] Execution did not return result.")
            except OrderExecutionError as e:
                log.error(f"[{symbol}] Execution failed: {e}")
            except Exception as e:
    ...
    async def start(self):
        ...
        if self.is_running:
            log.warning("TradingManager already running.")
            return
        ...
        async def simulate_data():
            while self.is_running:
                for symbol in self.trading_symbols:
                    ...
                    base_price = 50000 + (
                        symbol.__hash__() % 1000
                    )  # Different base per symbol
                    price = (
                        base_price
                        + math.sin(timestamp / (60 + (symbol.__hash__() % 30))) * 100
                    )
    ...
    async def stop(self):
        ...
        if not self.is_running:
            log.warning("TradingManager not running.")
            return
        ...
        tasks_to_cancel = [
            task for task in self._tasks.values() if task and not task.done()
        ]
        if tasks_to_cancel:
