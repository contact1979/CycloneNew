...
class Position:
    ...
    def update_position(
        self, fill_quantity: float, fill_price: float, timestamp: float
    ):
        """Updates the position based on a trade fill."""
        # Add validation for NaN/Inf values
        if (
            math.isnan(fill_quantity)
            or math.isnan(fill_price)
            or math.isinf(fill_quantity)
            or math.isinf(fill_price)
        ):
            log.error(
                f"[{self.symbol}] Invalid fill data: Qty={fill_quantity}, Price={fill_price}. Ignoring update."
            )
            return

        log.info(
            f"[{self.symbol}] Updating position: Qty={fill_quantity:.8f} @ Price={fill_price:.2f}"
        )
        ...
        # If opening or adding to position
            ...
            self.average_entry_price = (
                (old_value + new_value) / new_quantity if new_quantity != 0 else 0.0
            )
            self.quantity = new_quantity
            log.info(
                f"[{self.symbol}] Position updated: New Qty={self.quantity:.8f}, Avg Price={self.average_entry_price:.2f}"
            )
        # If reducing or flipping position
        else:
            new_quantity = self.quantity + fill_quantity
            if (self.quantity > 0 and new_quantity < 0) or (
...
class PositionManager:
        ...
        if self.config.redis and redis is not None:
            try:
                self.redis_client = redis.Redis(
                    ...
                    password=(
                        self.config.redis.password.get_secret_value()
                ...
                log.info(
                    f"Successfully connected to Redis: {self.config.redis.host}:{self.config.redis.port}"
                )
        ...
        try:
            ...
            for k, v in pos_dict.items():
                if isinstance(v, float) and (math.isnan(v) or math.isinf(v)):
                    log.warning(
                        f"[{position.symbol}] Cannot save NaN/Inf value for '{k}' to Redis. Storing as None."
                    )
        ...
        try:
            ...
            while cursor != 0:
                cursor, keys = self.redis_client.scan(
                    cursor=cursor, match=f"{self.redis_key_prefix}*", count=100
                )
                for key in keys:
                    try:
                        ...
                        if value:
                            ...
                            for k, v in pos_data.items():
                                if (
                                    v is None
                                    and k in Position.__annotations__
                                    and Position.__annotations__[k] == float
                                ):
                                    pos_data[k] = (
                                        0.0  # Restore floats from None (or NaN?)
                                    )
                        ...
                        log.exception(
                            f"Failed to load/parse position for key {key}: {e}"
                        )
        except Exception as e:
    ...
    def update_position_on_fill(
        self, symbol: str, quantity: float, price: float, timestamp: float
    ):
        ...
        position.update_position(
            fill_quantity=quantity, fill_price=price, timestamp=timestamp
        )
        self._save_position_to_redis(position)

    def update_position(
        self, symbol: str, trade: Dict[str, Any], current_price: float
    ) -> Position:
        ...
        timestamp = (
            trade.get("timestamp", datetime.utcnow()).timestamp()
            if isinstance(trade.get("timestamp"), datetime)
            else trade.get("timestamp", time.time())
        )
    ...
    def get_total_portfolio_value(
        self, current_prices: Dict[str, float], quote_currency: str = "USDT"
    ) -> float:
        ...
        for symbol, position in self.positions.items():
            if position.quantity != 0:
                ...
                if (
                    ...
                    log.warning(
                        f"[{symbol}] Cannot calculate value: Missing or invalid current price."
                    )

        # Placeholder for adding free quote currency balance
        log.debug(
            f"Calculated portfolio asset value: {total_value:.2f} {quote_currency}"
        )
        return total_value
