...
from ..config.settings import AppSettings, RiskSettings, get_config
...
class RiskController:
    ...
    def update_portfolio_value(self, current_value: float):
        ...
        if math.isnan(current_value) or math.isinf(current_value):
            log.error(
                f"Invalid current portfolio value received: {current_value}. Skipping drawdown check."
            )
            return
        ...
        if self.portfolio_peak_value > 0:
            ...
            if drawdown_pct >= self.risk_config.max_drawdown_pct:
                if not self.is_trading_halted:
                    log.critical(
                        f"MAX DRAWDOWN! Halting trades. Drawdown={drawdown_pct:.2%}"
                    )
                    self.is_trading_halted = True
            else:
                if (
                    self.is_trading_halted
                    and drawdown_pct < self.risk_config.max_drawdown_pct / 2
                ):
    ...
    def validate_trade(
        ...
        if math.isnan(current_portfolio_value) or math.isinf(current_portfolio_value):
            log.error(
                f"[{signal.symbol}] Cannot validate trade: Invalid portfolio value {current_portfolio_value}."
            )
            return None
        ...
        is_opening_or_increasing_trade = (
            (current_position_size == 0 and signal.action in ["BUY", "SELL"])
            or (current_position_size > 0 and signal.action == "BUY")
            or (current_position_size < 0 and signal.action == "SELL")
        )
        ...
        if is_opening_or_increasing_trade and signal.symbol not in open_positions:
            if num_open_positions >= self.config.trading.max_open_positions:
                log.warning(
                    f"[{signal.symbol}] Trade REJECTED: Max open positions ({self.config.trading.max_open_positions})."
                )
                return None
        ...
        if (
            ...
            # Validate inputs
            if (
                math.isnan(signal.stop_loss_price)
                or math.isnan(signal.price)
                or math.isnan(signal.quantity)
                or math.isinf(signal.stop_loss_price)
                or math.isinf(signal.price)
                or math.isinf(signal.quantity)
            ):
                log.error(
                    f"[{signal.symbol}] Cannot calculate risk: Invalid signal parameters (SL, price, or qty)."
                )
                return None

            if current_portfolio_value <= 0:
                log.warning(
                    f"[{signal.symbol}] Cannot calculate risk %: Portfolio value <= 0."
                )
            else:
                risk_per_unit = abs(
                    signal.price - signal.stop_loss_price
                )  # Use abs for simplicity
                total_risk_amount = risk_per_unit * abs(signal.quantity)
                risk_pct_of_portfolio = total_risk_amount / current_portfolio_value

                log.debug(
                    f"[{signal.symbol}] Proposed risk: Amount={total_risk_amount:.2f}, Pct={risk_pct_of_portfolio:.4%}"
                )

                if risk_pct_of_portfolio > self.risk_config.max_risk_per_trade_pct:
                    log.warning(
                        f"[{signal.symbol}] Risk per trade ({risk_pct_of_portfolio:.4%}) exceeds limit ({self.risk_config.max_risk_per_trade_pct:.4%})."
                    )
                    # Reduce size
                    allowed_risk_amount = (
                        current_portfolio_value
                        * self.risk_config.max_risk_per_trade_pct
                    )
                    if risk_per_unit > 1e-9:  # Avoid division by zero or tiny numbers
                        new_quantity = allowed_risk_amount / risk_per_unit
                        log.warning(
                            f"[{signal.symbol}] Reducing quantity: {signal.quantity:.8f} -> {new_quantity:.8f}"
                        )
                        signal.quantity = new_quantity
                    else:
                        log.error(
                            f"[{signal.symbol}] Cannot resize trade: Risk per unit too small. Rejecting."
                        )
                        return None
