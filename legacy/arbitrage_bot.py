...
async def execute_trade(buy_exchange_info, sell_exchange_info, opportunity):
    ...
    try:
        ...
        try:
            ...
            logging.info(f"Buy order {buy_order['id']} canceled on {buy_exchange.id}")
...
async def monitor_order(exchange, order_id, symbol, timeout=60):
    ...
    while (datetime.utcnow() - start_time).seconds < timeout:
        try:
            ...
            if order["status"] == "closed":
                ...
                logging.warning(f"Order {order_id} on {exchange.id} was canceled.")
                return False
        except Exception as e:
            logging.error(
    ...
    logging.warning(f"Order {order_id} on {exchange.id} not filled within timeout.")
    return False
...
async def main():
    ...
    try:
        while True:
            ...
            if opportunities:
                for opp in opportunities:
                    ...
                    # sell_currency assigned but not used
                    ...
                    if buy_balance_ok and sell_balance_ok:
                        ...
                        if success:
                            logging.info("Arbitrage trade executed successfully.")
                        else:
                            logging.warning("Arbitrage trade execution failed.")
                    else:
                        logging.warning(
        ...
        logging.error(f"An error occurred in the main loop: {type(e).__name__} - {e}")
    finally:
