...
def backup_config(backup_dir: Path = None) -> Path:
...
def restore_config(backup_path: Path) -> None:
    ...
    if env_backup.exists():
        ...
        print("Note: Secrets were not restored. Please update the .env file with your actual secrets.")
...
def main():
    parser = argparse.ArgumentParser(description="HydroBot Configuration Management Utility")
    ...
    restore_parser.add_argument('backup_path', type=str, help='Path to backup directory')
    
    # Create environment config command
    env_parser = subparsers.add_parser('create-env', help='Create environment-specific configuration')
