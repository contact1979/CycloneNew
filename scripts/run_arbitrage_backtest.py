...
class ArbitrageBacktester:
    ...
    def synchronize_data(self, data):
        ...
        logger.info(
            f"Data synchronized across {len(data)} exchanges, {len(df_merged)} valid data points"
        )
        return df_merged
    ...
    def initialize_balances(self):
        ...
        logger.info(
            f"Initialized balance of ${initial_balance} for {len(exchange_names)} exchanges"
        )
    ...
    def find_arbitrage_opportunities(self, row, timestamp):
        ...
        for buy_ex in exchange_names:
            for sell_ex in exchange_names:
                ...
                sell_fee = self.exchanges[sell_ex]["fees"]["taker"]                # Calculate maximum trade amount based on available balance
                max_amount_usd = self.balances[buy_ex]["USD"] / (
                    buy_price * (1 + buy_fee)
                )
                # max_amount_btc variable not used in calculation
                
                max_amount = min(
                    self.trade_amount, self.max_trade_amount, max_amount_usd
                )
    ...
    def calculate_performance(self):
        ...
        if num_trades > 0:
            ...
            max_profit = (
                max([t["actual_profit"] for t in self.trade_log])
                if num_trades > 0
                else 0
            )
            min_profit = (
                min([t["actual_profit"] for t in self.trade_log])
                if num_trades > 0
                else 0
            )
        else:
    ...
    def print_results(self, performance, trade_df):
        ...
        summary_data = [
            ...
            [
                "Total Profit",
                f"{profit_color}${performance['profit']:.2f}{Style.RESET_ALL}",
            ],
            [
                "Profit %",
                f"{profit_color}{performance['profit_percent']:.2f}%{Style.RESET_ALL}",
            ],
            ["Number of Trades", performance["num_trades"]],
            ["Total Fees Paid", f"${performance['total_fees_paid']:.2f}"],
            [
                "Avg Profit per Trade",
                f"{profit_color}${performance['avg_profit_per_trade']:.2f}{Style.RESET_ALL}",
            ],
        ]

        print(
            tabulate(summary_data, headers=["Metric", "Value"], tablefmt="fancy_grid")
        )
        ...
        if not trade_df.empty:
            ...
            sample_trades = (
                trade_df.sample(sample_size) if len(trade_df) > 5 else trade_df
            )

            # Format for display
            sample_trades_display = sample_trades.copy()
            sample_trades_display["timestamp"] = sample_trades_display[
                "timestamp"
            ].dt.strftime("%Y-%m-%d %H:%M:%S")
            sample_trades_display["profit"] = sample_trades_display[
                "actual_profit"
            ].map(lambda x: f"${x:.2f}")
        ...
        if not trade_df.empty:
            ...
            trade_df.to_csv(
                f"{results_dir}/arbitrage_trades_{timestamp}.csv", index=False
            )

            # Save performance summary
            with open(
                f"{results_dir}/arbitrage_performance_{timestamp}.json", "w"
            ) as f:
